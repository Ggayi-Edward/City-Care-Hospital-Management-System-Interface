...
const availability = {
  'Dr. Smith': { availableDays: [1, 3, 5], timeSlots: ['09:00 AM', '10:00 AM', '2:00 PM'] },
  'Dr. Adams': { availableDays: [2, 4], timeSlots: ['11:00 AM', '1:00 PM'] },
  'Dr. Green (Cardiologist)': { availableDays: [1, 2], timeSlots: ['10:00 AM', '3:00 PM'] },
  'Dr. Blue (Dermatologist)': { availableDays: [3, 5], timeSlots: ['09:30 AM', '1:30 PM'] },
};

const isDayAvailable = (doctor, date) => {
  if (!doctor || !availability[doctor]) return true;
  return availability[doctor].availableDays.includes(date.getDay());
};
...
export default function ReceptionistBookAppointment() {
  ...
  const handleSubmit = (e, form, setForm) => {
    e.preventDefault();
    const { patientName, date, time, doctor, reason } = form;
    if (!patientName || !date || !time || !doctor || !reason) {
      setMessage('Please fill out all fields.');
      return;
    }
    setMessage(`Appointment for ${patientName} (${activeTab}) booked successfully.`);
    setForm({ patientName: '', date: null, time: '', doctor: '', reason: '' });
  };
    ...
    return (
      ...
      >
        ...
        <input
          ...
          onChange={(e) => handleChange(form, setForm, 'patientName', e.target.value)}
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 font-roboto">
      <ReceptionistSidebar active="Book Appointment" />
